...
window.plugin.wasabee.loadExternals = function() {
    ...
    var GreatCircle = function (c, e, a) {
        this.start = c; this.end = e; this.properties = a || {}; a = this.start.x - this.end.x; a = Math.pow(Math.sin((this.start.y - this.end.y) / 2), 2) + Math.cos(this.start.y) * Math.cos(this.end.y) * Math.pow(Math.sin(a / 2), 2); this.g = 2 * Math.asin(Math.sqrt(a)); if (this.g == Math.PI) { throw Error("it appears " + c.view() + " and " + e.view() + " are 'antipodal', e.g diametrically opposite, thus there is no single route but rather infinite"); } if (isNaN(this.g)) {
            throw Error("could not calculate great circle between " +
                c + " and " + e);
        }
    };
    ...
    GreatCircle.prototype.Arc = function (c, e) {
        var a = []; if (2 >= c) { a.push([this.start.lon, this.start.lat]), a.push([this.end.lon, this.end.lat]); } else { for (var g = 1 / (c - 1), b = 0; b < c; b++) { var k = this.interpolate(g * b); a.push(k) } } for (var d = !1, h = 0, b = 1; b < a.length; b++) { var g = a[b - 1][0], k = a[b][0], m = Math.abs(k - g); 350 < m && (170 < k && -170 > g || 170 < g && -170 > k) ? d = !0 : m > h && (h = m) } g = []; if (d && 10 > h) {
            for (d = [], g.push(d), b = 0; b < a.length; b++) {
                if (k = parseFloat(a[b][0]), 0 < b && 350 < Math.abs(k - a[b - 1][0])) {
                ...
                } else { d.push([a[b][0], a[b][1]]); }
            }
        } else { for (d = [], g.push(d), b = 0; b < a.length; b++) { d.push([a[b][0], a[b][1]]); } } a = new Arc(this.properties); for (b = 0; b < g.length; b++) { for (k = new LineString, a.geometries.push(k), d = g[b], h = 0; h < d.length; h++) { k.move_to(d[h]); } } return a
